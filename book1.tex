...
%\textbf{Ex. 1} \\ $t^{3}x'+\sqrt{1-x^{2}} = 0$ \\
%\break \textbf{Solution:} \\

% $\int_{x_0}^{x} \frac{d\xi}{\sqrt{1-\xi^{2}}}= - \int_{t_0}^{t} \frac{1}{s^{3}}ds$ \\
% $\arcsin{x}=\frac{1}{2t^{2}}+c, c\in\R$ \\
 %$x = \sin{\frac{1}{2t^{2}}+c}$ \\


%\textbf{Ex. 2} \\ $\sqrt{t}x'=\sqrt{1+x^{2}}$ \\
%\break \textbf{Solution:} 
%Let $t=t(x)$.\\
%Then, as we solve the equation, we get:\\
%$-2\sqrt{t}=\ln(x+\sqrt{1+x^{2}})+c$, where $c\in\R$\\
%The solution is: $t=\frac{1}{4}(\ln(x+\sqrt{1+x^{2}})+c)^{2}$\\s
%\end{examples}
%aici scrii exercitii delia
%urmatorul ex ti-e in caiet!
...
\section{The General Form of a Linear Differential Equation of First Order}
...
\chapter{Linear Differential Systems}
...
\section{Matrix Analysis Theory}
...
\section{Existence and Unicity Theorems}
...
\textquotedblleft$\Rightarrow $\textquotedblright
  ...
  The lemma is proved.
\end{proof}
Resuming the demonstration of the theorem, let us denote $A:C(J,\R^{2})\rightarrow C(J,\R^{2})$, $u\longmapsto Au$, where:
...
\begin{theorem}[of existence, unicity and representation]
