...
\end{enumerate}
Let $\phi:G\times X\rightarrow X$ be an operator.
\begin{definition}
 The triplet $(X,G,\phi)$ is named a dynamic system if:
 \begin{enumerate}[i)]
  \item $\phi(0,x)=x,\forall x\in X$
  \item $\phi(t,\phi(s,x))=\phi(t+s,x),\forall t,s \in G, \forall x\in X$
 \end{enumerate}
\end{definition}
If $G=\mathbb{Z}$, then the system is called discrete. If $G=\R$, then the system is called continuous.
Space $X$ is called the phases space or the states space. Operator $\phi(t,\cdot):X\rightarrow X, t\in G$ is called the transition operator. Operator $\phi(\cdot,x):G\rightarrow X, x\in X$ is called the movement of x in regard with the dynamic system $(X,G,\phi)$. The set $\phi(G,X)$ is called the trajectory of $x$ or the orbit of $x$.
...
\begin{theorem}
 If $(X,G,\phi)$ is a dynamic system, then $\phi(t,\cdot):X\rightarrow X$ is a topological isomorphism, for all $t$ in $G$.
\end{theorem}
\begin{proof}
 We observe that $$\phi(-t,\cdot)\circ \phi(t,\cdot)=\phi(t,\cdot) \circ \phi (-t,\cdot)=1_{x},$$ for all $t\in G$. So $\phi(t,\cdot)$ is a bijection and $(\phi(t,\cdot))^{-1}=\phi(-t,\cdot)$. Knowing that $\phi$ is continuous and that $G$ is a topological group, results that operators $\phi(t,\cdot)$ and $\phi(-t,\cdot)$ are continuous. So $\phi(t,\cdot)$ is a topological isomorphism for all $t\in G$.
\end{proof}
